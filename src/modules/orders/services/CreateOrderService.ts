...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkInexistentProduct.length) {
      throw new AppError(
        `Could not find products ${checkInexistentProduct[0].id}`,
      );
    }
    ...
    if (findProductWithQuantityAvailable.length) {
      throw new AppError(
        `The quantity ${findProductWithQuantityAvailable[0].quantity} is not available for ${findProductWithQuantityAvailable[0].id}`,
      );
    }
    ...
    const orderedProductQauntity = order_products.map(product => ({
    ...
    await this.productsRepository.updateQuantity(orderedProductQauntity);
